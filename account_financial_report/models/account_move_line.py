...
class AccountMoveLine(models.Model):
    ...
    def _compute_analytic_account_ids(self):
        ...
        for record in self.filtered("analytic_distribution"):
            # NB: ``analytic_distribution`` is a JSON field where keys are:
            # - 'project.id' if only the project is set
            # - 'project.id,tag.id' if both project and analytic tag are set
            # Both "Project" and "Analytic Tag" column in the widget refer to
            # ``account.analytic.account`` records
